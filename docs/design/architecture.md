...
3. **Dummy Player tab** — tracks the active player-simulation's deck and draws; buttons for "run turn" and "end round / new round". The standard **Dummy Player** mode (see `CONTEXT.md`) is implemented: a Knight-select setup screen, then an AI screen that plays turns and ends rounds against a `DummyPlayerSession`. The tab also hosts a mode selector between that standard mode, **Volkare** (The Lost Legion's Volkare's Return/Volkare's Quest), and **Proxy Player** (a more elaborate, interactive Dummy Player replacement introduced in Apocalypse Dragon, but usable in any solo/coop scenario, not just the ones that expansion added) — one tab, one mode active per scenario, not three separate tabs. Volkare is now designed (see `CONTEXT.md`'s **Volkare Session** entry, [ADR-0004](../adr/0004-volkare-narrates-cards-not-simulates-board.md), and `docs/rules/volkares-return.md`/`volkares-quest.md`) but not yet implemented. Proxy Player is now designed too (see `CONTEXT.md`'s **Proxy Player Session** entry and `docs/rules/proxy-player.md`) but not yet implemented — like Volkare, it narrates rather than simulates, and reuses (via a card type shared with `DummyPlayerSession`) the standard Dummy Player's round-end Advanced Action/Spell offer interaction.
...
- The Dummy Player tab's Volkare and Proxy Player modes — both designed (see `CONTEXT.md`'s **Volkare Session**/**Proxy Player Session** entries, [ADR-0004](../adr/0004-volkare-narrates-cards-not-simulates-board.md), `docs/rules/volkares-return.md`/`volkares-quest.md`, and `docs/rules/proxy-player.md`) but not yet built.
