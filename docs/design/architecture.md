...
2. **Score** — the Score Calculator wizard. Solo scoring now covers every scenario with a working scoring engine in `domain/` (see `Scenario.entries`) - Solo Conquest, First Reconnaissance, For the Council, The Hidden Valley, The Realm of the Dead, Against the Dragon, Against the Horsemen, Apocalypse is Here, The Fractured Lands, Life and Death, The Lost Relic, Against the Apocalypse, and Solo Conquest Challenge (whose Knight-specific "Challenge" pages only appear for the 4 Knights - Arythea, Goldyx, Krang, Braevalar - whose objective needs a field no other Knight's does; `wizardPagesFor` takes both `Scenario` and `Knight` for this one scenario). Volkare's Quest/Return have scoring engines in `domain/` but aren't wired into this tab yet - see issue #106.
...
- Volkare's Quest and Volkare's Return — scoring engines exist in `domain/` but aren't wired into the Score Calculator tab yet (see issue #106).
