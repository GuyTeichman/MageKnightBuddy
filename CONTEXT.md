...
**Draw Log**:
The **Enemy Picker**'s newest-first record of every token drawn since the last Reset, persisted alongside the pile state. Load-bearing rather than decorative: because the picker discards a token the moment it's drawn and models no map, the log is the *only* record of which enemy is still standing on which space. Tapping an entry re-opens that token zoomed with its info window. Each entry carries a **defeated** flag: a freshly drawn enemy starts **on the board** (not defeated) because it was just revealed, and is marked defeated once the player beats it — the common reveal→defeat flow. The log groups the on-the-board enemies at the top and the defeated ones (dimmed) below; an on-board entry may carry a short free-text note ("keep, NE tile"). The flag is purely a memory aid — it has no effect on any **Token Pile** (a defeated enemy was already discarded when it was drawn), so draw odds are identical however it's set.
_Avoid_: History (reserved for the Scoreboard's saved sessions); treating the defeated flag as pile state; "still in play" (the flag's sense is now inverted — it tracks *defeated*, defaulting to on-the-board)
