 ...
 * screen). Reset clears what's persisted; an in-memory game survives until that screen is re-entered
 * and reloads from the (now empty) store. Live Room-Flow readers (Scoreboard, favorites) update at once.
 */
class AppReset(
    ...
    suspend fun resetToDefault() {
        withContext(Dispatchers.IO) {
            database.useWriterConnection { connection ->
                // usePrepared runs the SELECT and hands back a cursor-like statement; step() advances it
                // row by row, getText(0) reads this row's single `name` column into the collected list.
                val tableNames = connection.usePrepared(
                ...
                // Names are quoted defensively; DELETE (not DROP) keeps the schema, just empties the rows.
                tableNames.forEach { name -> connection.execSQL("DELETE FROM \"$name\"") }
            }
        }
