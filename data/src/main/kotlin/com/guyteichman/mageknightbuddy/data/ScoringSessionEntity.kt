...
/**
 * The Room-persisted form of a completed [com.guyteichman.mageknightbuddy.domain.ScoringSession].
 ...
 * [inputJson] holds the scenario's `*ScoringInput` encoded via [ScoringInputDto] (see that file
 * for why) - one JSON column instead of a separate wide column per field of every scenario's
 * differently-shaped input, which couldn't scale past Solo Conquest. The `toEntity`/`toDomain`
 * extension functions in ScoringSessionMapper.kt convert between the two shapes.
 */
