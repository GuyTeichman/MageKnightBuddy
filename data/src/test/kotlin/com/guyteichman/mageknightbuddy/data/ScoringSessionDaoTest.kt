...
class ScoringSessionDaoTest {
    ...
    @Test
    fun `deleteById removes only the row with that id, leaving the rest`() = runTest {
        val dao = database.scoringSessionDao()
        dao.insert(testEntity(playedAtEpochMillis = 1000L))
        dao.insert(testEntity(playedAtEpochMillis = 2000L))

        // Read the autogenerated ids back rather than assuming them; delete the newer row's.
        val newerId = dao.getAll().first().single { it.playedAtEpochMillis == 2000L }.id
        dao.deleteById(newerId)

        // Only the untouched older row survives - the delete is scoped to the one id.
        assertEquals(listOf(1000L), dao.getAll().first().map { it.playedAtEpochMillis })
    }
